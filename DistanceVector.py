...
class Node:
    def __init__(self,name,cols, rows, nodePlace):
        ...
        for x in range(cols):
            self.subChart[nodePlace][x] = -999
        for x in range(rows):
            self.subChart[x][nodePlace] = -999
        for x in range(rows):
            self.shortPath[x] = -999
    ...
    def updateShortPath(self, cols, rows):
        for x in range(rows):
            ...
            place = 9999
            for y in range(cols):
                if self.subChart[x][y]<smallDistance and self.subChart[x][y]!=-999 and self.subChart[x][y]!=-1:
                    smallDistance = self.subChart[x][y] 
                    place = y 
 
            if smallDistance!=9999 and smallDistance>0:
...
#         self.shortNode = {} 
    ...
    def getFinalResult(self,nodeNames): 
        ...
        for x in self.shortPath:
            if self.shortPath[x]!=-999:

                value = value+ "router "+self.name+": "
                val = str(self.shortPath[x])
                if self.shortPath[x]>=1000:
                    val = "INF"
                if val != "INF":
                    value = value + nodeNames[x]+" is "+ val+" routing through "+nodeNames[self.shortNode[x]]+"\n" 
                else:
                    value = value + nodeNames[x]+" is "+ val+"\n" 
...
def cauculate(fromNode, midNode):
    for path in midNode.shortPath:
        if midNode.shortPath[path]!=-999 and path!=nodeNames.index(fromNode.name):
            if fromNode.subChart[nodeNames.index(midNode.name)][nodeNames.index(midNode.name)]!=-999:
...
while len(inputFile)>0:
    ...
    #       route table
            ...
            if distance==-1:
                for x in range(int(nodeNumber)):
                    if nodes[nodeNames.index(eachLine[1])].subChart[x][nodeNames.index(eachLine[0])]!=-999:
                        nodes[nodeNames.index(eachLine[1])].subChart[x][nodeNames.index(eachLine[0])] = distance 
                    if nodes[nodeNames.index(eachLine[0])].subChart[x][nodeNames.index(eachLine[1])]!=-999:    
